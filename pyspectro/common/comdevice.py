        ...
        if not (self.parent):
            ...
            if comif:
                ...
                try:
                    ...
                    raise Exception(
                        "Error getting COM parameter '%s' from parent interface %s.\n %s"
                        % (self.name, self.parent.comif, str(e))
                    )
    ...
    def update_parameters(self, names=None, recursive=False):
        ...
        for name in names:
            ...
            try:
                ...
                logger.debug(
                    "Warning: could not update parameter '%s' from COM interface on model '%s'.\n %s"
                    % (name, self.fullname, str(e))
                )
        ...
        for name in names:
            ...
            if name in wnames:
                ...
                try:
                    ...
                    logger.debug(
                        "Warning: Writable parameter '%s' could not be set to '%s'.\n %s" % (name, modelVal, str(e))
                    )

                result = getattr(self.comif, name)
                if result != modelVal:
                    logger.debug(
                        "Warning: Attempt to set parameter %s to %s failed.  Actual value = %s"
                        % (name, modelVal, result)
                    )
